<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Gestión de Tipos de Curso</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra los tipos de cursos disponibles en el sistema</p>
        </div>

        <div class="flex flex-wrap gap-2">
            @can('tipocurso.export')
                <button
                    wire:click="export"
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Tipos de Curso</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $tipocursos->total() }} {{ $tipocursos->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                            ...
                            >
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">
                                                {{ strtoupper(substr($tipocurso->nombre_curso ?? $tipocurso->nombre ?? 'T', 0, 1)) }}
                                            </span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">
                                                {{ $tipocurso->nombre_curso ?? $tipocurso->nombre }}
                                            </div>
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('tipocurso.view')
                                            <a href="{{ route('tipocurso.show', $tipocurso) }}"
                                        ...
                                        @can('tipocurso.edit')
                                            <a href="{{ route('tipocurso.edit', $tipocurso) }}"
                                        ...
                                        @can('tipocurso.delete')
                                            <button
                                                wire:click="confirmTipoCursoDeletion({{ $tipocurso->id }}, '{{ addslashes($tipocurso->nombre_curso ?? $tipocurso->nombre ?? '') }}')"
                                ...
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400 mb-4 max-w-sm">
                                            ...
                                            @else
                                                Comienza agregando los tipos de cursos disponibles en el sistema.
                                            @endif
                                        </p>
                                        @if(!$search && auth()->user()->can('tipocurso.create'))
                                            <a href="{{ route('tipocurso.create') }}"
                                               class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                                </svg>
                                                Crear primer tipo de curso
                                            </a>
                                        @endif
                                    </div>
                                </td>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Tipo de Curso?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar <span class="font-semibold text-gray-900 dark:text-white">{{ $tipoCursoNameBeingDeleted }}</span>.
