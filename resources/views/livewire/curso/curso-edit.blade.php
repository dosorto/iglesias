    ...
    <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600
        ...
        <div class="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-white leading-tight">Editar Curso</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        @php
                            $badgeClass = match(strtolower($estado ?? '')) {
                                'activo'     => 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300',
                                'finalizado' => 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300',
                ...
                <div>
                    ...
                    <select wire:model="tipo_curso_id"
                        ...
                        @foreach($tipos as $t)
                            <option value="{{ $t->id }}">{{ $t->nombre_curso ?? $t->nombre }}</option>
                        @endforeach
                    </select>
                    @error('tipo_curso_id')
                        <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 flex items-center gap-1">
                ...
                <div>
                    ...
                    <select wire:model="instructor_ids" multiple
                            ...
                            class="w-full px-3 py-2.5 text-sm rounded-lg transition-colors
                                   ...
                                   text-gray-900 dark:text-white
                                   focus:ring-2 focus:ring-blue-500 focus:border-transparent
                                   @error('instructor_ids') border-red-400 @enderror">
                        @foreach($instructores as $i)
                            <option value="{{ $i->id }}">{{ $i->feligres?->persona?->nombre_completo }}</option>
                        @endforeach
                    </select>
