<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Detalle de Bautismo</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Información completa del registro bautismal</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <dl class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $bautismo->iglesia?->nombre ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Fecha</dt>
                    <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $bautismo->fecha_bautismo?->format('d/m/Y') ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Encargado</dt>
                    <dd class="text-sm font-medium text-gray-900 dark:text-white">{{ $bautismo->encargado?->feligres?->persona?->nombre_completo ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Libro</dt>
                    <dd class="text-sm font-mono text-gray-900 dark:text-white">{{ $bautismo->libro_bautismo ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Folio</dt>
                    <dd class="text-sm font-mono text-gray-900 dark:text-white">{{ $bautismo->folio ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500 dark:text-gray-400">Partida N°</dt>
                    <dd class="text-sm font-mono text-gray-900 dark:text-white">{{ $bautismo->partida_numero ?? '—' }}</dd>
                </div>
                @if($bautismo->observaciones)
                    <div>
                        <dt class="text-sm text-gray-500 dark:text-gray-400 mb-1">Observaciones</dt>
                        <dd class="text-sm text-gray-900 dark:text-white">{{ $bautismo->observaciones }}</dd>
                    </div>
                @endif
            </dl>
        </div>
