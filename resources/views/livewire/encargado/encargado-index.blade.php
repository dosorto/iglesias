<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Gestión de Encargados</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra los encargados registrados y su firma principal</p>
        </div>

        <div class="flex flex-wrap gap-2">
            @can('encargado.create')
                <a href="{{ route('encargado.create') }}"
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Encargados</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $encargados->total() }} {{ $encargados->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-100 dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Estado Encargado</span>
                            </th>
                            <th class="px-6 py-3 text-left">
                                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Firma Principal</span>
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($encargados as $item)
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-amber-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">{{ strtoupper(substr($item->feligres->persona->primer_nombre, 0, 1) . substr($item->feligres->persona->primer_apellido, 0, 1)) }}</span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">{{ $item->feligres->persona->nombre_completo }}</div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $item->estado === 'Activo' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' }}">
                                        {{ $item->estado }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-xs font-mono text-gray-500 dark:text-gray-400 truncate max-w-[180px] block">
                                        {{ $item->path_firma_principal ?? '—' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('encargado.view')
                                            <a href="{{ route('encargado.show', $item) }}"
                                        ...
                                        @can('encargado.delete')
                                            <button
                                                wire:click="confirmEncargadoDeletion('{{ $item->id }}', '{{ $item->feligres->persona->nombre_completo }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if(!$search && auth()->user()->can('encargado.create'))
                                            <a href="{{ route('encargado.create') }}"
                                               class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 mt-2">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                                </svg>
                                                Registrar primer encargado
                                            </a>
                                        @endif
                                    </div>
                                </td>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Encargado?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar a <span class="font-semibold text-gray-900 dark:text-white">{{ $encargadoNameBeingDeleted }}</span>.
