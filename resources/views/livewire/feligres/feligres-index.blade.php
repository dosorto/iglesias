            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Feligreses</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $feligres->total() }} {{ $feligres->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                            ...
                            >
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">{{ strtoupper(substr($item->persona?->primer_nombre ?? '?', 0, 1) . substr($item->persona?->primer_apellido ?? '', 0, 1)) }}</span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">{{ $item->persona?->nombre_completo ?? '(sin persona)' }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 font-mono">{{ $item->persona?->dni ?? '—' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-white">{{ $item->iglesia?->nombre ?? '—' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('feligres.view')
                                            <a href="{{ route('feligres.show', $item) }}"
                                        ...
                                        @can('feligres.edit')
                                            <a href="{{ route('feligres.edit', $item) }}"
                                        ...
                                        @can('feligres.delete')
                                            <button
                                                wire:click="confirmFeligresDeletion('{{ $item->id }}', '{{ $item->persona?->nombre_completo ?? 'Sin persona' }}')"
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if(!$search && auth()->user()->can('feligres.create'))
                                            <a href="{{ route('feligres.create') }}"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Feligrés?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar a <span class="font-semibold text-gray-900 dark:text-white">{{ $feligresNameBeingDeleted }}</span>.
