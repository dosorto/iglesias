<div class="space-y-6 sacramento-create-form">
    ...
    <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-violet-600 to-purple-600
        ...
        <div class="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-white leading-tight">Registrar Confirmación</h1>
                    <p class="text-violet-100 text-sm mt-0.5">Completa los pasos para registrar el acto de confirmación</p>
                </div>
            </div>
            ...
            @foreach ($pasos as $i => $p)
                <div class="flex flex-col items-center flex-shrink-0">
                    <div @class([
                        ...
                        'bg-gradient-to-br from-violet-500 to-purple-600 text-white shadow-violet-500/30'       => $paso === $p['n'],
                    ...
                    <span @class([
                        ...
                        'text-violet-600 dark:text-violet-400'       => $paso === $p['n'],
    ...
    <div class="space-y-4">
        ...
        @php
            $rolesConfig = [
                ['key' => 'confirmado', 'label' => 'Confirmado',  'required' => true,  'accent' => 'violet'],
                ['key' => 'padre',      'label' => 'Padre',       'required' => false, 'accent' => 'blue'],
                ...
                ['key' => 'ministro',   'label' => 'Ministro',    'required' => false, 'accent' => 'amber'],
            ];
        @endphp
        ...
        @foreach ($rolesConfig as $rc)
            @php
                ...
                $rolFelId  = $key === 'ministro' ? $this->ministro_feligres_id : $this->{"{$key}_feligres_id"};
            ...
            <div class="bg-white dark:bg-gray-800/80 rounded-xl shadow-sm border overflow-hidden
                ...
                <div class="flex items-center justify-between px-6 py-3.5
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        @if ($rc['required'])
                            <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-300">Obligatorio</span>
                        @else
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">Opcional</span>
                        @endif
                        @if ($rolEstado === 'found')
                            <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">&#x2713; Registrado</span>
                        @endif
                    </div>
                ...
                <div class="p-5 space-y-4">
                        ...
                        <div class="flex gap-3">
                            ...
                            <button type="button"
                                    ...
                                    class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold
                                           bg-violet-600 hover:bg-violet-700 text-white shadow-sm transition-all disabled:opacity-60">
                            ...
                            @foreach ($busqueda_resultados as $res)
                                <button type="button"
                                    ...
                                    <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-600
                                        ...
                                        <svg class="w-4 h-4 text-gray-400 group-hover:text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                  d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                        </svg>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $res['nombre_completo'] }}</p>
                                        <p class="text-xs text-gray-400 mt-0.5">
                                            DNI: {{ $res['dni'] }}
                                            @if ($res['telefono']) &middot; {{ $res['telefono'] }} @endif
                                        </p>
                                    </div>
                                    <svg class="w-4 h-4 text-gray-300 group-hover:text-violet-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if ($rolEstado === 'found')
                        <div class="flex items-center gap-3 p-4 rounded-xl
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-gray-900 dark:text-white truncate text-sm">{{ $rolPersona['nombre_completo'] }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    DNI: {{ $rolPersona['dni'] }}
                                    @if ($rolPersona['telefono']) &nbsp;&middot;&nbsp;{{ $rolPersona['telefono'] }} @endif
                                </p>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-semibold
                                        ...
                                        Persona encontrada &mdash; no registrada como feligrés
                                    </span>
                                </div>
                            </div>
                            @if (! $isMiniOpen)
                        ...
                        <div class="rounded-xl border border-emerald-200 dark:border-emerald-700/50
                            ...
                            <div class="flex items-center justify-between px-5 py-3
                                ...
                                <h4 class="text-sm font-semibold text-emerald-800 dark:text-emerald-300 flex items-center gap-2">
                                    ...
                                    Nuevo feligrés — <span class="font-bold">{{ $rc['label'] }}</span>
                                </h4>
                                <button type="button" wire:click="cancelarMini"
                                        class="p-1 rounded-md text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
                                </button>
                            ...
                            <div class="p-5 space-y-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div class="sm:col-span-2">
                                        ...
                                        <input type="text" wire:model="mini_p_dni" placeholder="Ej: 0801199912345"
                                               ...
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      ...
                                                      @error('mini_p_dni') border-red-400 @enderror" />
                                        @error('mini_p_dni') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Primer Nombre <span class="text-red-500">*</span></label>
                                        <input type="text" wire:model="mini_p_primer_nombre"
                                               ...
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                      focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                      @error('mini_p_primer_nombre') border-red-400 @enderror" />
                                        @error('mini_p_primer_nombre') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Segundo Nombre</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Primer Apellido <span class="text-red-500">*</span></label>
                                        <input type="text" wire:model="mini_p_primer_apellido"
                                               oninput="this.value=this.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s']/g,'').replace(/\b\w/g,c=>c.toUpperCase())"
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                      focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                      @error('mini_p_primer_apellido') border-red-400 @enderror" />
                                        @error('mini_p_primer_apellido') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Segundo Apellido</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Fecha de Nacimiento <span class="text-red-500">*</span></label>
                                        <input type="date" wire:model="mini_p_fecha_nacimiento"
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                      focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                      @error('mini_p_fecha_nacimiento') border-red-400 @enderror" />
                                        @error('mini_p_fecha_nacimiento') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Sexo <span class="text-red-500">*</span></label>
                                        <select wire:model="mini_p_sexo"
                                                class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                       bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                       focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                       @error('mini_p_sexo') border-red-400 @enderror">
                                            <option value="">Seleccionar...</option>
                                            <option value="M">Masculino</option>
                                            <option value="F">Femenino</option>
                                        </select>
                                        @error('mini_p_sexo') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Teléfono <span class="text-red-500">*</span></label>
                                        <input type="text" wire:model="mini_p_telefono"
                                               ...
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                      focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                      @error('mini_p_telefono') border-red-400 @enderror" />
                                        @error('mini_p_telefono') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">Correo Electrónico</label>
                                        <input type="email" wire:model="mini_p_email" placeholder="ejemplo@correo.com"
                                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white
                                                      focus:ring-2 focus:ring-emerald-500 focus:border-transparent
                                                      @error('mini_p_email') border-red-400 @enderror" />
                                ...
                                <div class="flex justify-end gap-3 pt-2 border-t border-emerald-100 dark:border-emerald-800/40">
                                    ...
                                    <button type="button" wire:click="guardarMiniPersona"
                                        ...
                                        <svg wire:loading wire:target="guardarMiniPersona" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                                        </svg>
                    ...
                    @if ($isMiniOpen && $mini_tipo === 'feligres')
                        <div class="rounded-xl border border-violet-200 dark:border-violet-700/50
                                    bg-gradient-to-b from-violet-50/80 to-transparent
                                    dark:from-violet-900/15 dark:to-transparent overflow-hidden">
                            <div class="flex items-center justify-between px-5 py-3
                                        border-b border-violet-100 dark:border-violet-800/40
                                        bg-violet-50 dark:bg-violet-900/20">
                                <h4 class="text-sm font-semibold text-violet-800 dark:text-violet-300 flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="p-5 space-y-4">
                                <p class="text-xs text-violet-600 dark:text-violet-400 flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    Se registrará en la parroquia activa del sistema.
                                </p>
                                <div class="flex justify-end gap-3 pt-2 border-t border-violet-100 dark:border-violet-800/40">
                                    <button type="button" wire:click="cancelarMini"
                                            class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium
                                                   bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600
                                                   text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 transition-all">
                                    ...
                                    <button type="button" wire:click="guardarMiniFeligres"
                                            ...
                                            class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold
                                                   shadow-md shadow-violet-500/30 transition-all
                                                   bg-gradient-to-r from-violet-500 to-violet-600 hover:from-violet-600 hover:to-violet-700
                                                   text-white disabled:opacity-60">
                                        <svg wire:loading wire:target="guardarMiniFeligres" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                                        </svg>
        ...
        @error('confirmado_dni')
            <div class="flex items-center gap-2 p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50">
    ...
    <div class="space-y-5">
        ...
        <div class="bg-white dark:bg-gray-800/80 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700/60 ring-1 ring-black/5 dark:ring-white/5">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100 dark:border-gray-700/60">
                ...
                <h2 class="text-sm font-semibold text-gray-800 dark:text-gray-100 tracking-wide uppercase">Resumen de la Confirmación</h2>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800/80 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700/60 ring-1 ring-black/5 dark:ring-white/5">
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <input type="date" wire:model.defer="fecha_confirmacion"
                           class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                  bg-gray-50 dark:bg-gray-700/60 text-gray-900 dark:text-white
                                  focus:ring-2 focus:ring-violet-500 focus:border-transparent
                                  @error('fecha_confirmacion') border-red-400 @enderror" />
                    @error('fecha_confirmacion') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" wire:model="lugar_confirmacion" readonly
                           class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                ...
                <div class="sm:col-span-3">
                    ...
                    <textarea wire:model="observaciones" rows="3"
                              ...
                              class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                     bg-gray-50 dark:bg-gray-700/60 text-gray-900 dark:text-white
                                     focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none"></textarea>
                </div>

                <div class="sm:col-span-3">
                    <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5 uppercase tracking-wide">Nota Marginal (descriptiva)</label>
                    <textarea wire:model="nota_marginal" rows="2"
                              placeholder="Describe la nota marginal..."
                              class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                     bg-gray-50 dark:bg-gray-700/60 text-gray-900 dark:text-white
                                     focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none"></textarea>
                </div>
    ...
    <div class="flex justify-between items-center gap-4">
        @if ($paso > 1)
            <button type="button" wire:click="anteriorPaso"
                    class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium
                           text-gray-700 dark:text-gray-200 rounded-lg border border-gray-300 dark:border-gray-600
        ...
        @else
            <button type="button" wire:click="guardar"
                ...
                <span wire:loading.remove wire:target="guardar">Registrar Confirmación</span>
