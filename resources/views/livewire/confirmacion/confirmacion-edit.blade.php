        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="hidden" name="iglesia_id" value="{{ $confirmacion->iglesia_id }}">
                </div>
                ...
                <div>
                    ...
                    @error('libro_confirmacion') <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea wire:model.live.debounce.400ms="observaciones" rows="2"
                        ...
                        <div class="text-left">
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                                ...
                                @if ($asignados->isNotEmpty())
                                    — <span class="text-emerald-600 dark:text-emerald-400 font-medium">{{ $asignados->count() }} asignado(s)</span>
                                @else
                        ...
                        @foreach ($rolesEdit as $rc)
                            ...
                            <div class="px-5 py-4 {{ $rolEstado === 'found' ? 'bg-emerald-50/40 dark:bg-emerald-900/5' : '' }}">
                                    ...
                                    <div class="flex gap-2 ml-11">
                                        ...
                                        <button type="button"
                                                ...
                                                class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold
                                                       bg-violet-600 hover:bg-violet-700 text-white transition-all disabled:opacity-60">
                                        ...
                                        @foreach ($busqueda_resultados as $res)
                                            <button type="button"
                                                    ...
                                                    class="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-left
                                                           ...
                                                           hover:bg-violet-50 dark:hover:bg-violet-900/20 hover:border-violet-300
                                                ...
                                                <svg class="w-3.5 h-3.5 text-gray-300 group-hover:text-violet-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if ($rolEstado === 'sin_feligres')
                                    <div class="ml-11 mt-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50">
                                        <p class="text-xs font-medium text-amber-700 dark:text-amber-300 mb-2">
                                            <strong>{{ $rolPersona['nombre_completo'] }}</strong> existe pero no es feligrés
                                        </p>
                                        @if (! $isMiniOpen)
                                            <button type="button" wire:click="abrirRegistrarFeligres('{{ $key }}')"
                                                    class="text-xs font-semibold text-amber-700 dark:text-amber-300
                                                           border border-amber-300 dark:border-amber-600 rounded-lg px-3 py-1.5
                                    ...
                                    <div class="ml-11 mt-2 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50 space-y-2">
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button" wire:click="buscarPersona('{{ $key }}')"
                                                    class="px-3 py-1.5 bg-violet-600 hover:bg-violet-700 text-white text-xs font-semibold rounded-lg">
                                                Buscar
                                            </button>
                                        </div>
                                        @if (! $isMiniOpen)
                                            <button type="button" wire:click="abrirCrearPersona('{{ $key }}')"
                                                    class="w-full py-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300
                                        ...
                                        <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            ...
                                            <div class="sm:col-span-2 flex justify-end gap-2 pt-2 border-t border-emerald-100 dark:border-emerald-800/40">
                                                <button type="button" wire:click="cancelarMini"
                                                        class="px-4 py-2 text-xs font-medium rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 transition-all">
                                                    Cancelar
                                                </button>
                                                <button type="button" wire:click="guardarMiniPersona"
                                                        wire:loading.attr="disabled" wire:target="guardarMiniPersona"
                                                        class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-lg
                                                               bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700
                                                               text-white transition-all disabled:opacity-60">
                                                    <svg wire:loading wire:target="guardarMiniPersona" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24">
                                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                                                    </svg>
                                ...
                                @if ($isMiniOpen && $mini_tipo === 'feligres')
                                    <div class="ml-11 mt-3 rounded-xl border border-violet-200 dark:border-violet-700/50 overflow-hidden">
                                        <div class="flex items-center justify-between px-4 py-2.5 bg-violet-50 dark:bg-violet-900/20 border-b border-violet-100 dark:border-violet-800/40">
                                            <span class="text-xs font-bold text-violet-800 dark:text-violet-300">
                                        ...
                                        <div class="p-4">
                                            <p class="text-xs text-violet-600 dark:text-violet-400 mb-3">Se registrará en la parroquia activa del sistema.</p>
                                            <div class="flex justify-end gap-2">
                                                <button type="button" wire:click="cancelarMini"
                                                        class="px-4 py-2 text-xs font-medium rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 transition-all">
                                                    Cancelar
                                                </button>
                                                <button type="button" wire:click="guardarMiniFeligres"
                                                        ...
                                                        class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-lg
                                                               ...
                                                               text-white transition-all disabled:opacity-60">
                                                    <svg wire:loading wire:target="guardarMiniFeligres" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24">
                                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                                                    </svg>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-4 border-t border-dashed border-gray-200 dark:border-gray-700/60">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" wire:model.live.debounce.400ms="nota_marginal"
                ...
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.400ms="lugar_expedicion"
                           placeholder="Ej: Parroquia San Pablo"
                           class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                  bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white placeholder-gray-400
                                  focus:ring-2 focus:ring-violet-500 focus:border-transparent
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <input type="number" min="1" max="31" wire:model.live.debounce.400ms="exp_dia" placeholder="Día"
                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white placeholder-gray-400
                                      focus:ring-2 focus:ring-violet-500 focus:border-transparent
                        ...
                        <input type="number" min="0" max="99" wire:model.live.debounce.400ms="exp_ano" placeholder="Año"
                               class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 dark:border-gray-600
                                      bg-white dark:bg-gray-700/60 text-gray-900 dark:text-white placeholder-gray-400
                                      focus:ring-2 focus:ring-violet-500 focus:border-transparent
                                      [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none
                    ...
                    <div class="flex gap-2 mt-1">
                        @error('exp_dia') <p class="text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                        @error('exp_mes') <p class="text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                        @error('exp_ano') <p class="text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>
                </div>
