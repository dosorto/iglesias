<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Gestión de Confirmaciones</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Administra los registros de confirmación de la parroquia</p>
        </div>
        <div class="flex flex-wrap gap-2">
            @can('confirmacion.create')
            ...
            <div class="bg-gray-50 dark:bg-gray-700 px-6 py-4 border-b border-gray-200 dark:border-gray-600">
                <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                    <div class="flex items-center gap-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Lista de Confirmaciones</h2>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                            {{ $confirmaciones->total() }} {{ $confirmaciones->total() === 1 ? 'registro' : 'registros' }}
                        </span>
                    </div>
                    <div class="flex items-center gap-3 flex-1 max-w-lg w-full">
                        <div class="relative flex-1">
                            ...
                            <input wire:model.live.debounce.300ms="search" type="text"
                                   ...
                                   placeholder="Buscar por confirmado, DNI o parroquia…">
                        </div>
                        ...
                        <button wire:click="export"
                                ...
                                style="background-color: var(--color-verde-esperanza)">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-100 dark:bg-gray-900">
                        <tr>
                            <th class="px-6 py-3 text-left"><span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Confirmado</span></th>
                            ...
                            <th class="px-6 py-3 text-left"><span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ministro</span></th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse ($confirmaciones as $c)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-violet-600 rounded-full flex items-center justify-center mr-3 shadow-sm group-hover:scale-110 transition-transform">
                                            <span class="text-white text-xs font-bold">
                                                {{ strtoupper(substr($c->feligres?->persona?->primer_nombre ?? '?', 0, 1) . substr($c->feligres?->persona?->primer_apellido ?? '', 0, 1)) }}
                                            </span>
                                        </div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white">
                                                {{ $c->feligres?->persona?->nombre_completo ?? '—' }}
                                            </div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400 font-mono">
                                                {{ $c->feligres?->persona?->dni ?? '' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                {{-- Parroquia --}}
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-white">{{ $c->iglesia?->nombre ?? '—' }}</span>
                                </td>

                                {{-- Fecha --}}
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900 dark:text-white">
                                        {{ $c->fecha_confirmacion?->format('d/m/Y') ?? '—' }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-600 dark:text-gray-300">
                                        {{ $c->ministro?->persona?->nombre_completo ?? '—' }}
                                    </span>
                                </td>

                                {{-- Libro / Partida --}}
                                <td class="px-6 py-4">
                                    @if ($c->libro_confirmacion || $c->partida_numero)
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            @if ($c->libro_confirmacion) <span>{{ $c->libro_confirmacion }}</span> @endif
                                            @if ($c->folio) <span class="ml-1">Fol. {{ $c->folio }}</span> @endif
                                            @if ($c->partida_numero) <span class="ml-1">Ptda. {{ $c->partida_numero }}</span> @endif
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <div class="flex items-center space-x-2">
                                        @can('confirmacion.view')
                                            <a href="{{ route('confirmacion.show', $c) }}"
                                        ...
                                        @can('confirmacion.edit')
                                            <a href="{{ route('confirmacion.edit', $c) }}"
                                        ...
                                        @endcan
                                        @can('confirmacion.delete')
                                            <button
                                                wire:click="confirmConfirmacionDeletion({{ $c->id }}, '{{ addslashes($c->feligres?->persona?->nombre_completo ?? 'este registro') }}')"
                                ...
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
                                            @if($search) No se encontraron resultados @else No hay confirmaciones registradas @endif
                                        </h3>
                                        @if(!$search)
                                            @can('confirmacion.create')
                                                <a href="{{ route('confirmacion.create') }}"
                                                   class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors mt-2">
                                                    ...
                                                    Registrar primera confirmación
                                                </a>
                                            @endcan
                                        @endif
                                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-gray-200 dark:border-gray-700 anim-scale-in">
                <div class="p-6">
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Confirmación?</h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            Estás a punto de eliminar el registro de
                            <span class="font-semibold text-gray-900 dark:text-white">{{ $confirmacionNameBeingDeleted }}</span>.
