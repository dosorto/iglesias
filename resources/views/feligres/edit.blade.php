...
<div class="container-fluid max-w-4xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Feligrés</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <form action="{{ route('feligres.update', $feligre) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="id_persona" id="id_persona" required
                        ...
                        @foreach($personas as $persona)
                            <option value="{{ $persona->id }}" {{ old('id_persona', $feligre->id_persona) == $persona->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="id_iglesia" id="id_iglesia" required
                        ...
                        @foreach($iglesias as $iglesia)
                            <option value="{{ $iglesia->id }}" {{ old('id_iglesia', $feligre->id_iglesia) == $iglesia->id ? 'selected' : '' }}>
                                {{ $iglesia->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_ingreso" id="fecha_ingreso"
                           value="{{ old('fecha_ingreso', $feligre->fecha_ingreso?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <select name="estado" id="estado" required
                        ...
                        <option value="Activo" {{ old('estado', $feligre->estado) == 'Activo' ? 'selected' : '' }}>Activo</option>
                        <option value="Inactivo" {{ old('estado', $feligre->estado) == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                    </select>
                    @error('estado')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
