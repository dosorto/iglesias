...
<div class="container-fluid max-w-4xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Nuevo Tipo de Curso</h1>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <form action="{{ route('tipocurso.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nombre_curso" id="nombre_curso" value="{{ old('nombre_curso') }}" required
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="descripcion_curso" id="descripcion_curso" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white @error('descripcion_curso') border-red-500 @enderror">{{ old('descripcion_curso') }}</textarea>
                ...
                <div>
                    ...
                    <select name="estado_curso" id="estado_curso" required
                        ...
                        <option value="activo" {{ old('estado_curso') == 'activo' ? 'selected' : '' }}>Activo</option>
                        <option value="inactivo" {{ old('estado_curso') == 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                    </select>
                    @error('estado_curso')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
