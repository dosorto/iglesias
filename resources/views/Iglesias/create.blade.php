    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <form action="{{ route('iglesias.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- Párroco --}}
                <div>
                    <label for="parroco_nombre" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Nombre del Párroco <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="parroco_nombre" id="parroco_nombre" value="{{ old('parroco_nombre') }}" required
                           ...
                           placeholder="Nombre completo del párroco">
                ...
                {{-- Religión --}}
                <div>
                    <label for="id_religion" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Religión <span class="text-red-500">*</span>
                    </label>
                    <select name="id_religion" id="id_religion" required
                            class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white @error('id_religion') border-red-500 @enderror">
                        <option value="">— Selecciona una religión —</option>
                        @foreach ($religiones as $religion)
                            <option value="{{ $religion->id }}" {{ old('id_religion') == $religion->id ? 'selected' : '' }}>
                                {{ $religion->religion }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="direccion" id="direccion" rows="3" required
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white @error('direccion') border-red-500 @enderror">{{ old('direccion') }}</textarea>
                    @error('direccion')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
