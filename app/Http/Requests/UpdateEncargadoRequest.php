...
class UpdateEncargadoRequest extends FormRequest
{
    ...
    {
        $encargadoId = $this->route('encargado')?->id ?? $this->route('encargado');

        return [
            'id_feligres'          => ['required', 'integer', 'exists:feligres,id', Rule::unique('encargado', 'id_feligres')->ignore($encargadoId)->whereNull('deleted_at')],
